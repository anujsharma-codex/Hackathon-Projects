...
AI Prompt: "Write a Streamlit app for a solar panel health monitoring dashboard. It should: 1) Allow CSV upload or use demo data, 2) Show metrics in columns (total, healthy, degrading, end-of-life, financial loss), 3) Display a histogram and pie chart of panel health, 4) Allow searching individual panels and show details with QR code, 5) Show recommendations summary table. Use plotly for charts."
'''
...
if load_demo:
    if os.path.exists(DEMO_PATH):
        try:
            st.session_state["df"] = load_csv(DEMO_PATH)
...
if df is not None:
    ...
    # ---------- SIGNATURE: FLEET HEALTH HORIZON ----------
    total = len(filtered)
    keep_pct = filt_summary["healthy_count"] / total * 100
    repurpose_pct = filt_summary["degrading_count"] / total * 100
    recycle_pct = filt_summary["end_of_life_count"] / total * 100
    ...
    m1.metric("Total Panels", f"{filt_summary['total_panels']:,}")
    m2.metric("Healthy (≥85%)", f"{filt_summary['healthy_count']:,}")
    m3.metric("Degrading (70–85%)", f"{filt_summary['degrading_count']:,}")
    m4.metric("End-of-Life (<70%)", f"{filt_summary['end_of_life_count']:,}")
    ...
    with tab_explorer:
        ...
        with sort_col:
            sort_by = st.selectbox("Sort list by", ["panel_id", "efficiency", "health_score", "remaining_life"])
    ...
    with tab_financial:
        ...
        f2.metric("Panels to Replace", f"{filt_summary['end_of_life_count']:,}")
        f3.metric("Repurpose Potential", f"{filt_summary['repurpose_recommendations']:,}")
        saved = filt_summary["repurpose_recommendations"] * 10000
